...
class Evaluator(object):
    ...
    def _evaluate_multi_classification(self, h:torch.tensor, t:torch.tensor):
        ...
        true = float(torch.where(h==t, 1, 0).sum())
        false = float(torch.where(h!=t, 1, 0).sum())

        # raw accuracy
        acc = float(true/(true+false+1e-12))
    ...
    def _evaluate_multi_classification_with_rejection(self, h:torch.tensor, t:torch.tensor, r_binary:torch.tensor):
        ...
        t = float(torch.where(h_rjc==t_rjc, 1, 0).sum())
        f = float(torch.where(h_rjc!=t_rjc, 1, 0).sum())
        # accuracy
